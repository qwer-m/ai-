# AI Test Platform

一个基于 AI 的自动化测试辅助平台，支持测试用例生成、UI 自动化测试、API 测试管理等功能。
特别⚠️当前只有生成测试用例可以用，UI自动化测试和接口自动化测试还在开发中。你可以根据你的项目去上传历史的一些需求文档和测试用例来当做RAG知识库，填入配置⚙️里边的api，然后就可以用了。

## 功能特性

- 🤖 **AI 测试生成**：基于需求文档自动生成测试用例
- 🖥️ **UI 自动化**：支持 Playwright 录制与回放
- 🔌 **API 测试**：接口测试管理与自动化执行
- 📊 **测试报告**：生成详细的测试执行报告
- 🧠 **知识库**：基于 RAG 的测试知识管理

## 核心架构 (Core Architecture)

本项目采用 **"总指挥 + 三大副指挥" (Commander & Deputies)** 的多 Agent 协作架构，实现从需求分析到测试执行的全流程智能化。

### 1. 总指挥 (Commander)
- **战略大脑**: 由 `Meta-Analysis Agent` 担任，负责在执行具体任务前进行战略规划。
- **动态决策**: 自动分析系统类型 (Web/Mobile/IoT)、影响范围 (Feature/Regression) 和复杂度。
- **策略分发**: 动态生成测试策略 (如 60% 功能测试 / 20% 回归测试 / 20% 安全测试)，指导下游 Agent 执行。

### 2. 三大副指挥 (Execution Agents)
- **QA 工程师 Agent (TestGen)**: 
  - 负责测试用例生成。
  - 集成 RAG 知识库，支持上下文压缩与双重索引。
  - 严格遵循总指挥下发的测试策略与比例。
- **视觉感知 Agent (UI Automation)**:
  - 负责 UI 自动化测试。
  - 具备视觉定位能力，不依赖传统 XPath，直接识别界面元素。
  - 自动感知设备环境 (Android/Web)。
- **API 专家 Agent (API Testing)**:
  - 负责接口自动化测试。
  - 生成 Pytest 脚本，支持 Mock 数据构造与 Fuzzing 测试。
  - 自省式调试，自动捕获请求细节。

### 3. 质量审计 (Evaluation)
- 独立的 **审计员 Agent**，对生成的测试用例和脚本进行质量打分与覆盖率分析，确保输出质量。

## 快速开始

### 前置要求

- **操作系统**: 需要在电脑上安装 Ubuntu (推荐使用 WSL 子系统)
- Python 3.10+
- Node.js 16+
- MySQL 8.0+
- Redis

### 1. 后端设置

```bash
# 进入项目目录
cd ai_test_platform

# 安装依赖
pip install -r requirements.txt

# 配置环境变量
# 复制 .env.example 为 .env 并填入您的配置
cp ../.env.example .env

# 初始化数据库 (确保 MySQL 已启动并创建了数据库)
# 您可能需要根据 models.py 自动创建表，或使用 alembic 迁移
```

### 2. 前端设置

```bash
# 进入前端目录
cd frontend

# 安装依赖
npm install

# 启动开发服务器
npm run dev
```

### 3. 启动服务

**方式一：Windows 一键启动 (强烈推荐)**
进入 `ai_test_platform` 文件夹，直接双击运行 **`启动服务.bat`** 脚本。
该脚本会自动检测虚拟环境并启动所有服务（后端、Celery、前端）。

**方式二：命令行启动**
在根目录运行：
```bash
python ai_test_platform/start_dev.py
```

**方式二：手动启动**
需要分别启动以下服务：
1. Backend API: `uvicorn main:app --reload`
2. Celery Worker: `celery -A celery_worker.celery_app worker -P solo`
3. Frontend: `npm run dev`

## 部署

本项目包含 Docker 部署配置，支持一键部署到服务器。

```bash
# 部署脚本位于 deploy/ 目录
./deploy/deploy_aliyun.sh
```

## 技术栈

- **Frontend**: React, TypeScript, Vite, Ant Design
- **Backend**: FastAPI, SQLAlchemy, Celery
- **AI/ML**: LangChain, ChromaDB, DashScope
- **Infrastructure**: Docker, Nginx, MySQL, Redis
