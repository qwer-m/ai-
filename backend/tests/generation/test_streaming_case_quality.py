...
from modules.test_generation_components.postprocess.streaming_case_quality import (
    diagnose_final_quality_cases,
    final_quality_diagnostic_reason,
...
def test_final_quality_diagnostic_reason_reports_invalid_expected_quality_marker() -> None:
    assert final_quality_diagnostic_reason({"expected_result_quality": "invalid_case"}) == "expected_result_quality:invalid_case"


def test_final_quality_diagnostic_reason_does_not_trust_stale_expected_quality_marker() -> None:
    assert (
        final_quality_diagnostic_reason(
            {
...
def test_final_quality_diagnostic_reason_detects_reasoning_truncation_and_non_assertable_text() -> None:
    assert final_quality_diagnostic_reason({"expected_result": "需求未明确时先按默认配置"}) == "reasoning_leakage"
    assert final_quality_diagnostic_reason({"expected_result": "操作后应正常展"}) == "truncated_text"
    assert final_quality_diagnostic_reason({"expected_result": "result is as configured"}) == "non_assertable_expected_result"
    assert final_quality_diagnostic_reason({"expectedResult": "result is as configured"}) == "non_assertable_expected_result"
    assert final_quality_diagnostic_reason({"expected_result": "系统提示“课程保存成功”"}) == ""
...
def test_diagnose_final_quality_cases_keeps_text_heuristic_hits_and_records_diagnostic() -> None:
    ...
    retained, diagnostic_total = diagnose_final_quality_cases(
        [valid_case, "not-a-case", non_assertable_case],
        low_quality_drop_details,
        stage="final_quality_gate",
    )

    assert retained == [valid_case, non_assertable_case]
    assert diagnostic_total == 1
...
def test_filter_low_quality_cases_with_stats_keeps_weak_cases_for_global_review() -> None:
    weak_case = {
        "id": "TC-WEAK",
        "test_module": "Report",
    ...
    assert [case["id"] for case in filtered] == ["TC-WEAK", "TC-KEEP"]
    assert stats["invalid_structure_dropped"] == 2
    assert stats["weak_case_dropped"] == 0
    assert stats["weak_case_detected"] == 1
    ...
    assert stats["diagnostic_details"][0]["reason"] == "missing_expected_result"


def test_filter_low_quality_cases_with_stats_leaves_similarity_to_global_review() -> None:
    ...
    assert {case["id"] for case in filtered} == {"TC-DEDUP-A", "TC-DEDUP-B", "TC-UNIQUE"}
