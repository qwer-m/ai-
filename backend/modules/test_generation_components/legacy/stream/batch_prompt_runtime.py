...
from ...postprocess.module_contract import functional_architecture_generation_context


def build_recent_history_context(history_summaries: list[str]) -> str:
    if not history_summaries:
        return ""
    history_list_str = "\n".join([f"- {item}" for item in history_summaries])
    return f"""
                    ...
                    Focus on NEW scenarios across the complete verified functional architecture.
                    """
...
def build_stream_batch_system_prompt(
    *,
    ...
    return f"""
                ...
                4. Optimize coverage across the complete verified architecture; do not force equal counts per module.
...
def build_functional_architecture_instruction(*, project_profile: dict) -> str:
        ...
        "module_interactions": [
            {
                ...
                "source_module_key": str(item.get("source_module_key") or ""),
                "target_module_key": str(item.get("target_module_key") or ""),
                "trigger": str(item.get("trigger") or ""),
                "transferred_entity": str(item.get("transferred_entity") or ""),
    ...
    return f"""
# --- VERIFIED FUNCTIONAL ARCHITECTURE (GLOBAL BATCH SCOPE) ---
