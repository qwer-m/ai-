...
class LegacyGenerationContextHybridMixin:
    def _resolve_kb_context_with_hybrid(
        ...
        _ = compress  # preserve signature compatibility
        ...
        if status_messages is not None:
            status_messages.append("preparing project snapshot context...")
        ...
        if snapshot_result.get("success") and (snapshot_result.get("snapshot_text") or "").strip():
            ...
            if status_messages is not None:
                if snapshot_result.get("cache_hit") and snapshot_usable:
                    ...
                    status_messages.append(
                        f"snapshot updated -> reusing latest snapshot ({snapshot_result.get('rebuild_reason', 'unknown')})"
                    )

        fallback_reason = ""
        if not snapshot_text:
            fallback_reason = str(snapshot_result.get("fallback_reason") or "snapshot_unavailable")
            ...
            if status_messages is not None:
                if queue_result.get("queued"):
                    ...
                    status_messages.append("snapshot pending -> waiting skipped, using rag_only (reason=already_pending)")
        ...
        if status_messages is not None:
            if use_rag:
                status_messages.append(f"precision retrieval enabled ({','.join(precision_reasons)})")
            else:
                status_messages.append("precision retrieval not triggered, snapshot-only mode")
        ...
        if use_rag:
            try:
                ...
                try:
                    ...
                    if status_messages is not None:
                        status_messages.append(f"RAG retrieval failed ({e}), snapshot-only fallback")
            ...
            if strategy == "sync_snapshot_retry_then_fail" and HYBRID_EMPTY_GUARD_CONFIG.sync_snapshot_retry_enabled:
                ...
                if retry_result.get("success"):
                    ...
                    if not retry_empty.get("hybrid_empty_context"):
                        ...
                        abort_error = "empty context even after sync snapshot retry"
                    ...
                    abort_error = f"sync snapshot retry failed: {sync_err}"
                ...
                abort_generation = True
                abort_error = "empty context: snapshot and rag unavailable"
        ...
        if status_messages is not None:
            status_messages.append(
                "hybrid fusion done: "
                f"mode={fusion_debug.get('fusion_mode')} "
                f"rag_chunks={fusion_debug.get('rag_chunk_count', 0)} "
                f"tokens~{fusion_debug.get('final_context_tokens', 0)}"
            )
            if abort_generation:
                status_messages.append(
                    "hybrid empty guard triggered: "
                    f"final_decision={fusion_debug.get('final_decision')} "
                    f"reason={fusion_debug.get('hybrid_empty_reason') or fallback_reason}"
                )
