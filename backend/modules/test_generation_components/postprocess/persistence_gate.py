...
def summarize_persistence_case_quality_gate(
    ...
    *,
    ...
    if final_duplicate_count > max_final_duplicates:
        add_failure("final_scenario_duplicates_above_threshold")
    if final_misordered_count > max_final_misordered:
        add_failure("final_flow_misordered_above_threshold")
    if reasoning_leak_count > 0:
        add_failure("reasoning_leakage_detected")
    if role_mismatch_count > max_role_mismatch:
        add_failure("role_mismatch_above_threshold")
