...
class UIAutomationModule:
    ...
    def generate_ui_script(self, task_description: str, url: str, automation_type: str = "web", db: Session = None, user_id: int = None, requirement_context: str = None) -> str:
        ...
        if automation_type == "web":
            ...
            IMPORTANT:
            - After every major action (click, fill, navigate), you MUST print a JSON log entry to stdout.
            ...
            async def main():
                async with async_playwright() as p:
                    ...
                    try:
                        print(json.dumps({"type": "step", "action": "start", "details": "Browser launched", "status": "success"}))
                        await page.goto('https://example.com')
                        
                        # Take screenshot for AI analysis
                        screenshot_path = f"screenshot_{int(time.time()*1000)}.png"
                        await page.screenshot(path=screenshot_path)
                        print(json.dumps({"type": "screenshot", "path": screenshot_path, "details": "Page loaded"}))
                        ...
                        print(json.dumps({"type": "step", "action": "click", "details": "Clicked Login Button", "status": "success"}))
                        ...
                        print(json.dumps({"type": "error", "details": str(e)}))
                    finally:
                        await browser.close()
    ...
    def execute_script(self, script: str, url: str, task_description: str, automation_type: str = "web", db: Session = None, project_id: int = None, user_id: int = None, test_case_id: int = None) -> dict:
        """
        ...
        try:
            ...
            # Parse screenshots from stdout (looking for JSON logs)
            screenshot_paths = []
            try:
                for line in stdout.split('\n'):
                    if '"type": "screenshot"' in line or '"screenshot":' in line:
                        try:
                            # Try to extract JSON from line if it's mixed with other text
                            json_str = line
                            if '{' in line:
            ...
            if db:
                ...
                # Auto-evaluate quality after execution
                if status == "success" or status == "failed": # Evaluate even if failed to analyze why
                    try:
                        from modules.testing.evaluation import evaluator
                        # Use a simplified evaluation for now or async task
                        # Here we just trigger it but handle exceptions so it doesn't fail the execution response
                        eval_result = evaluator.evaluate_ui_automation(
                            script, result_text, db=db, project_id=project_id, user_id=user_id
                        )
                        execution.evaluation_result = eval_result
                        
                        # Parse score from eval result (heuristic)
                        score = 5.0 # Default
...
"""
        ...
        # Fallback: raise error if insertion fails
