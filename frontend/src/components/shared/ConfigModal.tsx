...
type CurrentConfigResponse = {
  ...
  turbo_model_name?: string;
  review_model_name?: string;
  has_api_key?: boolean;
  base_url?: string;
  turbo_follow_main?: boolean;
  turbo_provider?: string;
  has_turbo_api_key?: boolean;
  review_follow_main?: boolean;
  review_provider?: string;
  has_review_api_key?: boolean;
  vl_follow_main?: boolean;
  vl_provider?: string;
  has_vl_api_key?: boolean;
  vl_base_url?: string;
  tesseract_path?: string;
  tesseract_manual_override?: boolean;
  ocr_auto_detected?: boolean;
  ocr_auto_detect_message?: string;
};

type DetectServicesResponse = {
  services: DetectedService[];
};

type OcrDetectResponse = {
  found?: boolean;
  validated?: boolean;
  path?: string;
  message?: string;
...
const parseTestStreamEvent = (line: string): TestStreamEvent => {
  const parsed: unknown = JSON.parse(line);
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed) || !('type' in parsed)) {
    throw new Error('配置测试流返回了无效事件');
  }
...
export function ConfigModal({ show, onHide, initialError }: Props) {
  ...
  const [streamState, setStreamState] = useState<'idle' | 'running' | 'done' | 'error' | 'cancelled'>('idle');
  const testStreamAbortRef = useRef<AbortController | null>(null);
  ...
  useEffect(() => {
    if (!show) {
      testStreamAbortRef.current?.abort();
    }
  }, [show]);

  useEffect(() => () => testStreamAbortRef.current?.abort(), []);
  ...
  useEffect(() => {
    ...
    let mounted = true;
    api
      .get<CurrentConfigResponse>('/api/config/current')
      .then((res) => {
  ...
  const detectLocalServices = async () => {
    ...
    try {
      ...
      const res = await api.post<DetectServicesResponse>('/api/config/detect', { candidates });
  ...
  const autoDetectOcr = async () => {
    ...
    try {
      const res = await api.post<OcrDetectResponse>('/api/config/ocr/auto-detect', {});
  ...
  const runTestStream = async (payload: ConfigPayload) => {
    testStreamAbortRef.current?.abort();
    const controller = new AbortController();
    testStreamAbortRef.current = controller;
    setStreamState('running');
    setStreamOutput('');
    try {
      const response = await api.raw('/api/config/test-stream', {
        method: 'POST',
      ...
      const consumeLine = async (rawLine: string) => {
        ...
        const event = parseTestStreamEvent(line);
        if (event.type === 'token') {
          setStreamOutput((prev) => prev + event.token);
          return;
        }
        terminalEventReceived = true;
        if (event.type === 'error') {
          const text = await translateConfigError(event.message);
      ...
      if (testStreamAbortRef.current === controller) {
        testStreamAbortRef.current = null;
      }
    }
  };

  const cancelTestStream = () => {
    testStreamAbortRef.current?.abort();
  ...
  const validateConnection = async () => {
    ...
    try {
      const res = await api.post<ValidationResponse>('/api/config/validate', payload);
      ...
      if (res.valid) {
        ...
        const checksText = checks.length > 0 ? `，通过项：${checks.map((x) => `${x.label}:${x.model}`).join(' / ')}` : '';
        setMessage({
          type: 'success',
        ...
        const failedChecks = checks.filter((x) => !x.success);
        const suffix =
          failedChecks.length > 0
            ? `（失败项：${failedChecks.map((x) => `${x.label}:${x.model}`).join(' / ')}）`
            : '';
  ...
  const saveConfig = async () => {
    ...
    try {
      ...
      const res = await api.post<SaveConfigResponse>('/api/config/save', payload);
          ...
          <section className="config-preview-card config-bottom-split__left">
            <div className="config-preview-card__header d-flex justify-content-between align-items-center mb-2">
              ...
              <div className="d-flex align-items-center gap-2">
                ...
                {streamState === 'cancelled' && <Badge bg="secondary">已停止</Badge>}
              </div>
            </div>
