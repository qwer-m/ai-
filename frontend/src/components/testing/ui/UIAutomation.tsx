...
import {
  executionDetailFailureMessage,
  executionFailureMessage,
  parseConvertResponse,
  parseDetectResponse,
  parseDirectExecutionResponse,
  parseImportedCasesResponse,
  parseNaturalRunResponse,
  parseUIExecutionDetail,
  parseUIExecutionHistory,
  type ExportInfo,
  type NaturalLanguageOperation,
  type UIExecutionDetail,
  type UIExecutionSummary,
} from '../../UIAutomation/responseContracts';
...
export function UIAutomation({ projectId, projectName = '', onLog, view = 'web' }: Props) {
  ...
  const [selectedExecution, setSelectedExecution] = useState<UIExecutionDetail | null>(null);
  ...
  useEffect(() => {
    ...
    void api.get<unknown>(`/api/ui-automation/history?project_id=${projectId}`)
      .then(parseUIExecutionHistory)
      .then(setReportRows)
  ...
  useEffect(() => {
    ...
    if (executionId && (status === 'running' || status === 'pending')) {
      timer = setInterval(() => {
        void api.get<unknown>(`/api/ui-automation/${executionId}`).then(parseUIExecutionDetail).then((detail) => {
          setStatus(detail.status || 'failed');
          if (detail.status === 'failed') setExecutionError(executionDetailFailureMessage(detail));
        }).catch((error) => {
          const message = getErrorMessage(error);
          setExecutionError(message);
          onLog(`读取 UI 自动化执行状态失败：${message}`);
        });
      }, 2000);
    }
    return () => {
  ...
  const ensureTargetReady = async () => {
    ...
    const result = parseDetectResponse(await api.post<unknown>('/api/ui-automation/detect', {
      type: automationType,
  ...
  const executeOperation = async () => {
    ...
    try {
      ...
      const executionResponse = await api.raw('/api/ui-automation/execute', {
        method: 'POST',
        body: form,
      });
      const executionPayload: unknown = await executionResponse.json();
      const result = parseDirectExecutionResponse(executionPayload);
      const nextStatus = result.status;
      setStatus(nextStatus);
      setExecutionId(result.execution_id);
      setExportInfo(result.export);
      if (nextStatus === 'failed') setExecutionError(executionFailureMessage(result));
  ...
  const uploadTestCases = async (file: File) => {
    ...
    try {
      ...
      form.append('project_id', String(projectId));
      const response = parseImportedCasesResponse(
        await api.upload<unknown>('/api/ui-automation/import-test-cases', form),
      );
  ...
  const convertSelectedCases = async () => {
    ...
    try {
      ...
      for (const testCase of selected) {
        ...
        try {
          const preview = parseNaturalRunResponse(await api.post<unknown>('/api/ui-automation/natural-run', {
          ...
          const response = parseConvertResponse(await api.post<unknown>('/api/ui-automation/convert', {
            project_id: projectId,
            operation_name: operationName,
            operation_steps: testCase.steps,
            task: caseTask,
          ...
          setSelectedCaseId(response.test_case_id);
          successCount += 1;
        } catch (error) {
  ...
  const runNaturalLanguage = async () => {
    ...
    try {
      ...
      const response = parseNaturalRunResponse(await api.post<unknown>('/api/ui-automation/natural-run', {
        project_id: projectId,
        operation_name: name,
      ...
      if (nextStatus === 'success') {
        ...
        const reason = executionFailureMessage(result);
        ...
        <aside className="ui-automation-report-sidebar border-end overflow-auto">
          ...
          {reportRows.map((row) => (
            <button
              ...
              onClick={() => {
                void api.get<unknown>(`/api/ui-automation/${row.id}`)
                  .then(parseUIExecutionDetail)
                  .then(setSelectedExecution)
                  .catch((error) => onLog(`加载执行报告失败：${getErrorMessage(error)}`));
              }}
            >
