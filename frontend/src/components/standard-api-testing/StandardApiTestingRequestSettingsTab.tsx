...
function SettingRow({
  ...
  return (
    <div className={`d-flex align-items-center justify-content-between mb-3 pb-3 ${bordered ? 'border-bottom' : ''}`}>
      <div>
      ...
      <div className="p-4" style={{ maxWidth: '800px' }}>
        ...
        <SettingRow title="HTTP Version (HTTP 版本)" description="选择发送请求时使用的 HTTP 版本。">
          <div style={{ width: '150px' }}>
            <Form.Select size="sm" value={requestSettings.httpVersion} onChange={(e) => setRequestSettings({ ...requestSettings, httpVersion: e.target.value })}>
        ...
        {requestSettings.followRedirects && (
          <div className="d-flex align-items-center justify-content-between mb-3 pb-3 border-bottom ps-4">
            ...
            <div style={{ width: '100px' }}>
              <Form.Control size="sm" type="number" value={requestSettings.maxRedirects} onChange={(e) => setRequestSettings({ ...requestSettings, maxRedirects: parseInt(e.target.value) || 0 })} />
            </div>
          </div>
        )}
        ...
        <SettingRow title="Request Timeout (请求超时)" description="设置请求超时时间（毫秒，0 表示无限）。">
          <div style={{ width: '100px' }}>
            <Form.Control size="sm" type="number" value={requestSettings.timeout} onChange={(e) => setRequestSettings({ ...requestSettings, timeout: parseInt(e.target.value) || 0 })} />
          </div>
        </SettingRow>
        ...
        <div className="mb-3 pb-3 border-bottom">
          ...
          <Form.Control size="sm" placeholder="" value={requestSettings.disabledSSLProtocols} onChange={(e) => setRequestSettings({ ...requestSettings, disabledSSLProtocols: e.target.value })} />
        </div>
        ...
        <div className="mb-3 pb-3 border-bottom">
          ...
          <Form.Control as="textarea" size="sm" placeholder="Enter cipher suites" value={requestSettings.cipherSuites} onChange={(e) => setRequestSettings({ ...requestSettings, cipherSuites: e.target.value })} />
        </div>
      </div>
    </div>
  );
