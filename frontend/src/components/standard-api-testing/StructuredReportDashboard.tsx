    ...
    <div className="d-flex flex-column gap-3 animate-fade-in p-3">
      <Row className="g-3">
        <Col md={3} xs={6}>
          <Card className="text-center h-100 border-success shadow-sm">
            <Card.Body className="d-flex flex-column justify-content-center py-2">
              <h3 className="text-success mb-0 standard-api-fw-600">{Math.round(passRate)}%</h3>
        ...
        <Col md={3} xs={6}>
          <Card className="text-center h-100 border-primary shadow-sm">
            <Card.Body className="d-flex flex-column justify-content-center py-2">
              <h3 className="text-primary mb-0 standard-api-fw-600">{report.total}</h3>
        ...
        <Col md={3} xs={6}>
          <Card className={`text-center h-100 shadow-sm ${report.failed > 0 ? 'border-danger bg-danger bg-opacity-10' : 'border-light'}`}>
            <Card.Body className="d-flex flex-column justify-content-center py-2">
              <h3 className={`mb-0 standard-api-fw-600 ${report.failed > 0 ? 'text-danger' : 'text-secondary'}`}>{report.failed}</h3>
      ...
      {report.failures.length > 0 ? (
              ...
              <Accordion.Item eventKey={String(idx)} key={idx}>
                <Accordion.Header>
                  <div className="d-flex align-items-center gap-2">
                    ...
                    <span className="font-monospace text-truncate standard-api-report-failure-name">{fail.name}</span>
                  </div>
                </Accordion.Header>
                <Accordion.Body className="bg-light p-2">
                  <div className="mb-2 small">
                    <strong className="text-secondary">错误原因：</strong>
                    <span className="text-danger ms-2 standard-api-fw-500">{fail.message}</span>
                  </div>
        ...
        <div className="alert alert-success d-flex align-items-center mt-2">
          ...
          <div>
            <span className="standard-api-fw-600">测试通过！</span>
