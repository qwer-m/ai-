...
export function TestGenerationResultSection({ mode, result, streamingContent, loading, statsCount, onCopy }: TestGenerationResultSectionProps) {
  return (
    <div className="test-generation-result test-generation-result-panel bento-card col-span-12 p-0 overflow-hidden d-flex flex-column">
      <div className="test-generation-result-head bg-light border-bottom d-flex justify-content-between align-items-center px-4 py-3">
        ...
        <div className="d-flex align-items-center gap-2">
          ...
          {streamingContent ? (
            <Button variant="link" size="sm" className="p-0 text-decoration-none d-flex align-items-center gap-1 text-primary" onClick={onCopy} title="复制内容">
              <FaCopy /> 复制
            </Button>
          ) : null}
        </div>
